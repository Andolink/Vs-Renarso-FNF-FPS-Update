...
class HealthIcon extends FlxSprite
{
	...
	public function new(char:String = 'face', isPlayer:Bool = false, ?_id:Int = -1)
	{
		...
		if(Config.betterIcons){
			...
			animation.add('renarso', [24, 25, 24], 0, false, isPlayer);
			animation.add('bf-demongf', [28, 29, 28], 0, false, isPlayer);
			animation.add('renarsoevil', [26, 27, 26], 0, false, isPlayer);
			animation.add('gf-outside', [16], 0, false, isPlayer);
			animation.add('renarsocrazy', [24, 25, 24], 0, false, isPlayer);
		}
		else{
